readin_prokka_tables <- function(directory, recursive=T){
  ...
  gene_files <- dir(path = directory, recursive= recursive, pattern="tsv")
  nuc_seq <- dir(path = directory, recursive = recursive, pattern="ffn")
  prot_seq <- dir(path = directory, recursive = recursive, pattern="faa")
  ...
  for(i in 1:length(gene_files)){
    print(paste0("loading ... ", file.path(directory,gene_files[i])))
    #i=1
    int <- read.delim(file=file.path(directory, gene_files[i]),stringsAsFactors = F) %>%
      mutate(filename=gene_files[i])
    #glimpse(int)
    
    dflist[[i]] <- int
    
    print(paste0("loading ... ", file.path(directory,nuc_seq[i])))
    intn <- read.delim(file=file.path(directory, nuc_seq[i]),stringsAsFactors = F,header=F) %>%
    ...
    intp <- read.delim(file=file.path(directory, prot_seq[i]), stringsAsFactors = F,header=F) %>%
      mutate(seq_name=str_extract(V1,">.+")) %>%
      fill(seq_name) %>%
      filter(!grepl(V1,pattern="^>")) %>%
      group_by(seq_name) %>%
